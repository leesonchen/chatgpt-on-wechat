...
提醒内容："""
    ...
    def get_goal_suggestion_prompt(description: str) -> str:
        ...
        """
        prompt = f"""你是一个专业的目标管理顾问，请分析用户设定的目标："{description}"
...
请用80字以内回复，语气专业而温暖，包含具体可行的建议。

分析和建议："""
        return prompt

    @staticmethod
    def get_goal_validation_prompt(description: str) -> str:
        """
        生成目标验证的提示词

        专门用于检查目标内容是否合适的AI提示词。
        ...
        Returns:
            str: 用于验证目标适宜性的AI提示词
        """
        prompt = f"""请评估以下目标是否合适："{description}"
...
如果目标合适，请回复"✅ 目标合适"并给出简短鼓励。
如果目标需要改进，请回复"⚠️ 建议优化"并提供具体建议。
如果目标不当，请回复"❌ 不建议"并说明原因。
