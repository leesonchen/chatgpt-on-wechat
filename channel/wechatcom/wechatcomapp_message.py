...
class WechatComAppMessage(ChatMessage):
    def __init__(self, msg, client: WeChatClient, is_group=False, kf_msg=None):
        ...
        self.customer_service_mode = kf_msg is not None

        if self.customer_service_mode:
        ...
        if self.msgtype == "text":
            ...
            self.content = kf_msg['text']['content'] if self.customer_service_mode else msg.content
        elif self.msgtype == "voice":
            self.ctype = ContextType.VOICE
            logger.debug(f"[wechatcom] voice message: {msg}")
            self.media_id = kf_msg['voice']['media_id'] if self.customer_service_mode else msg.media_id
            media_format = ".mp3" if self.customer_service_mode else msg.format
            ...
            self.media_id = kf_msg['image']['media_id'] if self.customer_service_mode else msg.media_id
            media_format = ".jpg" if self.customer_service_mode else ".png"
